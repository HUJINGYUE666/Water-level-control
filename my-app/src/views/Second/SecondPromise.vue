<template>
  <div id="app">
    ...
    <div id="myChart2" :style="{width: '5rem', height: '4rem'}"></div>
    <common-footer :menu="menu2" :selectMenu="selectMenu" @change="fn"></common-footer>
  </div>
</template>
<script>
  ...
  created() {
    axios.post("/api/second").then(result => {
      ...
      result.data.word.forEach((val, index) => {
        this.chart.push(val);
        if (
          val.photo_dushu > val.station_baozheng_value &&
